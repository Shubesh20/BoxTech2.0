...
pointCloudSize = prod(floor(((qlim(2:6,2)-qlim(2:6,1))/stepRads + 1)+(qlim(1,2)-qlim(1,1))));
pointCloud = zeros(floor(pointCloudSize), 3); % Adjusting point cloud size to include the reduction
counter = 1;

tic
% Steps through the joint limits at n increments
for q1 = qlim(1,1):0.1:qlim(1,2) % First joint is rail and therefore will not have stepRad interval
            ...
            for q4 = qlim(4,1):stepRads*n:qlim(4,2)
                for q5 = qlim(5,1):stepRads*n:qlim(5,2)
                    for q6 = qlim(6,1):stepRads*n:qlim(6,2)
                        q7 = 0; % Rotation of end effector makes no difference on position
                        ...
                        if mod(counter/pointCloudSize * 100,1) == 0
                            display(['After ',num2str(toc),' seconds, completed ',num2str(counter/(pointCloudSize/(n)) * 100),'% of poses']);
                        end
                    end
                end
            end
...
convezHullPlot = trisurf(convexHull,pointCloud(:,1),pointCloud(:,2),pointCloud(:,3),'FaceColor','green');
fprintf('- \nVolume: \n%gm^3\n-\n', volume);

drawnow();

% Maximum range Calculations in X,Y,Z
